...
def ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    if stats.ships_left > 0:
        # Decrease number of ships left (lives).
        stats.ships_left -= 1
        # Update scoreboard
        sb.prep_ships()
...
def create_fleet(ai_settings, screen, ship, aliens):
    """Create a full fleet of windows."""
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    for bullet in bullets.copy(): # since it it bad practice to edit a for loop in progress loop through a copy
...
def check_play_button(ai_settings, screen, stats, sb, play_button, ship,
    ...
    if button_clicked and not stats.game_active:    
        if play_button.rect.collidepoint(mouse_x, mouse_y):
            ...
            sb.prep_level()
            sb.prep_ships()
            # Empty the lists of aliens an bullets
            aliens.empty()
            bullets.empty()
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens,
                  bullets, play_button):
    """Update images on the screen and render new screen on top."""
    # Redraw the screen during each loop pass-over
...
def check_high_score(stats, sb):
    ...
    Save new high scores, display new high scores as they appear"""
    if stats.score > stats.user_high_score:
            with open('current_highest_score.txt', 'w') as hs:
